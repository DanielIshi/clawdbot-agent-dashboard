...
test.describe('SessionSidebar', () => {
  test.beforeEach(async ({ page }) => {
    ...
    // Navigate to Settlers view (where SessionSidebar is)
  ...
  test('AC7: displays empty state when no sessions', async ({ page }) => {
    ...
    // Warte auf nächsten Polling-Request (10s + Buffer)
   ...
   * Main testing in Unit Tests (useSessionPolling.test.ts)
   * E2E: Just verify sessions update after navigation
   */
  test('AC6: sessions are fetched and displayed', async ({ page }) => {
    // Sessions sollten bereits geladen sein (siehe beforeEach)
    const cards = page.locator('[data-testid="session-card"]')
    await expect(cards).toHaveCount(3)

    // Verify first session visible
    const firstCard = cards.first()
